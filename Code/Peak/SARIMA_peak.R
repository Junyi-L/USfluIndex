...
seasonally_prediction_target <-
  ts(logit_prediction_target,
     frequency = 52)

sarima_fit <- readRDS(file = here("./Results/Peak/sarima_fit.rds"))
...
for(analysis_time_season in analysis_seasons) {
  ...
  for(analysis_time_season_week in seq(from = first_analysis_time_season_week, to = last_analysis_time_season_week - 1)) {
    ...
    ## Update anova fit object with data up
    ## through analysis_time_ind
    new_data <- seasonally_prediction_target[
      seq_len(max(0, analysis_time_ind))]
    
    updated_sarima_fit_try <- try(Arima(
      new_data,
      order = model[1:3],
      seasonal = list(order = model[4:6], period = model[7]),
