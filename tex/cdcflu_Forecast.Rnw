...
\abstract{
  ...
  % Results:
  Short-term flu activity was equally well predicted up to four weeks ahead 
  by the beta model with four autoregressive lags and by KCDE,
  however the beta model runs much faster. Non-dynamic Prophet scored worst.
...
The outcomes of flu infections vary widely and serious infections can cause hospitalization or death.
The Centers for Disease Control and Prevention (CDC) in the U.S. estimated that around 8\% of the U.S.
population became infected with influenza during an average season \cite{Tokars2017}.
...
Reich et al. \cite{Reich2019} compared the forecast accuracy of 22 models from five different institutions.
Biggerstaff et al. \cite{biggerstaff.etal2018} presented the result of a recent FluSight challenge.
Some of the most common approaches can be grouped into following categories \cite{brooks.etal2018}:
...
a wide range of epidemiological analyses, including forecasts of
infectious disease counts \cite{held.meyer2019}.
However, when measuring and forecasting flu activity, the proportion of outpatient visits
with ILI is often used rather than surveillance counts, and the total number of outpatient visits is
...
Furthermore, kernel conditional density estimation \cite[KCDE]{ray.etal2017},
a successful competitor in the FluSight challenge, is also included in our comparison.
...
Gneiting and Katzfuss \cite{gneiting.katzfuss2014} stated that
...
The MMWR weeks range from 1 to 52, sometimes to 53.
...
Historically, flu activity in MMWR week 40 to MMWR week 20 of the following year is of interest to the disease control purpose. 
...
In this paper we index seasons from MMWR week 31 to MMWR week 30 of the following year. 
...
Two H1N1 pandemic seasons, 2008/2009 and 2009/2010, are excluded from our analysis,
...
They can be divided into two categories: short-term targets and seasonal targets. 
The short-term targets are one-week, two-weeks, three-weeks and four-weeks ahead forecasts of the wILI. 
The seasonal targets consist of peak week timing and intensity prediction. 
The peak week of an influenza season is the week which has the highest wILI of the season. 
...
Dawid-Sebastiani score \cite{dawid.sebastiani1999} and absolute error of the test period or a subset of the test period. 
We also repeat the calculation of the maximum log score in \cite{ray.etal2017}.
...
It should only be considered when a model has good overall performance, 
as measured by the mean log score \cite{ray.etal2017}.
...
The bins are $[0,0.5 \%)$, $[0.5 \%,1 \%)$,$\dots$, $[13 \%, 100\%]$. 
...
\label{subsec:beta}
Lu and Meyer \cite{beta:theory} proposed an extension of the HHH framework 
...
terms of the odds ratio \cite{beta:theory}.
...
in the R package \texttt{forecast} \cite{hyndman.khandakar2008}. 
...
They evaluated different KCDE model specifications and in most cases of their 
application, the KCDE model with periodic kernel components and a full bandwidth 
matrix has a better forecast performance than the other model specifications.
...
Since the Prophet model assumes Gaussian errors, we fit the data on the logit scale. 
The same holiday effects as included in the beta model and the ARIMA model are taken into account.

The fifth baseline model is a naive approach: for each test season
we estimate a logitnormal distribution of the wILI based on the same 
calendar week in the previous seasons.


For seasonal targets we consider an additional reference approach which 
assigns equal probabilities to all possible outcomes. 
For prediction of peak week timing, probabilities are assigned equally to 
...
consequently increasing the wILI \cite{brooks.etal2018}. 
...
for the covariate $\bm{z}_t^{(v)}$ and 
exclude time trend in the harmonic regression in the beta model.
...
In this case, AICc is a more suitable criterion \cite{HURVICH1989, Burnham2002}.
...
Source code and data are available on GitHub \cite{ForecastCode}. 
The code for the KCDE method comes from \cite{reichlab}.
...
KCDE or ARIMA respectively in one- to four-weeks-ahead forecasts in 
...
print(xtable(res_ph1_4_2, align = "ccc|rrrrrr",
             caption = 'Summaries of model performance for short-term targets. Ranks are shown in brackets. 
...
print_2heading_xtable(res_ph1_4,
                      ...
                      Ranks are shown in brackets. The "all weeks" group summarizes 
...
KCDE or ARIMA for prediction horizons one to four in the "MMWR weeks 40-20" subset of the test period. 
...
for predictions in weeks before the actual peak of the given season. 
...
for both seasonal targets and in both subsets.
...
KCDE follows closely behind. The rank of other models is similar.
...
KCDE ranks first and is followed by naive. Only these two models are better than equal bin.

Season 2016/2017 is a relatively normal season. 
All models provide better predictions than equal bin for both targets. 
Two non-dynamic models, Prophet and naive, have better peak intensity predictions than other models.
For peak timing prediction, KCDE has the best score and is closely followed by prophet and naive. 
...
Other models have worse scores than equal bin. 
...
\centering
\caption{Each boxplot (top) summarizes predictions made by a model in the subset "before peak".
...
Positive values indicate the case in which the model outperforms the equal bin approach.
...
\section{Discussion}
\label{sec:Diss}
In this paper, the forecast performance for both short-term and seasonal 
targets of Beta(4) is compared with some commonly used models and a competitive model, the KCDE model.

Regarding to short-term prediction for both subsets, Beta(4) has the best 
...
93 times the run time of Beta(4). 
The forecast performance of Beta(4) decreases over prediction horizon. 
The non-dynamic models, Prophet and naive, do not show any competence in the short-term forecast.
...
with test seasons. 
For peak intensity prediction, two non-dynamic models, Prophet and naive, 
have better rank in normal seasons (season 2014/2015 and season 2016/2017) 
than in the other seasons (season 2015/2016 and season 2017/2018).
In contrast, KCDE scores best in both unusual seasons. 
For peak timing prediction, KCDE has the best performance in season 2015/2016,
when the wILI peaked relatively late. 
In other seasons with normal peak timing, SARIMA, KCDE or beta(1) make the best prediction.
In short, KCDE has the best forecast performance for both seasonal targets 
when the epidemic curve is unusual, and Prophet has the best peak intensity 
prediction during standard seasons.

We only include well-documented models and KCDE in our model comparison. 
There are some other competitive models presented in the FluSight challenge, 
...
Thus we do not include machine learning methods into our comparison. 
...
multi-model ensemble \cite{Reich2019, ray.reich2018}.
Forecasts of seasonal targets usually have worse scores than
short-term forecasts (Table \ref{tab:res_ph1_4_2} and Table \ref{tab:res_peak}).  
As discussed above, in seasonal forecasting KCDE outperforms other methods during unusual seasons and 
Prophet has the best peak intensity prediction during standard seasons. 
Thus multi-model ensembles with time-varying weights, e.g. \cite{ray.reich2018}, 
are promising method to combine the advantages of these models. 
For example, when predicting peak intensity, more weight could be assigned to Prophet 
when available observations indicate a standard season and vice versa. 
...
In conclusion, the beta model is competitive in short-term forecasting with 
simple structure and short run time, and KCDE is a robust 
method for seasonal forecasts. A multi-model ensemble is promising regards to seasonal forecasting.
