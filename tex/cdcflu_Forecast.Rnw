...
The code for KCDE method comes from \cite{reichlab}.
...
print(xtable(res_ph1_4_2, align = "ccc|rrrrrr",
             caption = 'Summaries of model performance for short-term targets. Ranks are shown in bracket. 
...
when wILI peaked relatively late. 
In other seasons with normal peak timing, SARIMA, KCDE or beta(1) makes the best prediction.
...
Forecasts of seasonal targets have usually worse scores than
...
is promising to combine the advantages of these models. 
...
method for seasonal forecasts. Multi-model ensemble is promising in seasonal forecasting.
