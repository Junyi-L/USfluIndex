...
through the testing period since Prophet needs to updates its change 
...
For higher prediction horizons, KCDE outperforms Beta(4) in all but the last testing season.
...
KCDE or ARIMA for prediction horizons one to four in the "MMWR weeks 40-20" subset in the testing period. 
...
KCDE has the best performance in terms of mean 
log score. 
...
The ARIMA model has the best forecast performance in the "before peak" subset. 
Then follow KCDE, Prophet, naive approach and Beta(4).

We now focus on the forecast performance in the subset "before peak"
during each testing seasons.
...
and Figure \ref{fig:Peakbox}), while in many training seasons
...
than the most training seasons. Only the KCDE model
produces better peak timing prediction than the equal bin approach in this season.
Furthermore, Prophet and naive provide 
worse peak intensity prediction than equal bin in this season. 
Five dynamic models have better peak intensity prediction than equal bin. 
Season 2016/2017 is a relatively normal season. Two non-dynamic models, Prophet
...
Season 2017/2018 has a relatively high peak intensity compared to all training seasons.
All models but KCDE has worse peak intensity prediction than equal bin. 
The peak timing of this season is normal, and all models have better peak timing prediction 
than equal bin, while SARIMA is worse than other models.
