...
In short-term prediction for both subsets, the Beta(4) model has the best 
...
with testing seasons. 
Two non-dynamic models, Prophet and naive, show better forecast 
performance than dynamic models when the prediction targets are relatively normal in comparison to
training seasons, e.g. for peak intensity prediction in season 2014/2015, 
and for both targets in season 2016/2017. 
In a season which has an unusual peak timing or peak intensity, non-dynamic models
have worse forecast performance than the equal bin approach, e.g. for 
...
than equal bin across all testing seasons and for both targets. 
